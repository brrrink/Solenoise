...
// - Added 200 uS delay to the gate reading to allow analog CV input to settle
...
#define GATE_SETTLE_DELAY_US 400   // Reduced slightly; tune as needed
...
void handleCVGate() {
  ...
  if (last_gate_state == HIGH && gate_state == LOW) {
    // Very short non-blocking wait using micros() instead of delay
    unsigned long startWait = micros();
    while (micros() - startWait < GATE_SETTLE_DELAY_US) {
      // Optionally yield or handle MIDI here for very high rates
      handleMIDI();  // Keep MIDI responsive during wait
    }

    // Re-sample CV after settling
    cv_value = analogRead(CV_PIN);

    int index = (cv_value * 12) / 1024;  // 0-11
...
void readInputs() {
  ...
  handleCVGate();   // CV/Gate now uses non-blocking wait
}
...
void updateDisplay() {
  ...
  display.print(step + 1);   // Show 1-12
  ...
  // Output status - supports multiple simultaneous active outputs
  for (int i = 0; i < 12; i++) {
    int x = i * 10;
