...
#define PULSE_DURATION 30 // Hold output HIGH for XXms (same for MIDI and CV/Gate)
...
int cv_value = 0;
int gate_state = HIGH;
int last_gate_state = HIGH;  // For edge detection on gate
...
unsigned long pulseStartTimes[12] = {0};  // Shared for both MIDI and CV pulses
  ...
  while (midiIn.read()) {
    ...
    if (type == midi::NoteOn || type == midi::NoteOff) {
      ...
      if (note >= MIDI_NOTE_START && note <= MIDI_NOTE_END && isOn) {
        ...
        pulseStartTimes[index] = millis();  // Start pulse timer
      }
    }
  }
}
...
void handleCVGate() {
  ...
  // Trigger on falling edge of gate (active low)
  if (last_gate_state == HIGH && gate_state == LOW) {
    // Divide 0-1023 into 12 equal ranges
    int index = (cv_value * 12) / 1024;  // 0 to 11

    int pin = OUTPUT_PIN_START + index;
    digitalWrite(pin, HIGH);
    pulseStartTimes[index] = millis();  // Start pulse timer (same as MIDI)
...
void updateDisplay() {
  ...
  // CV/Gate - now showing voltage range to user (0-5V)
  display.setCursor(0, 8);
  display.print("CV: ");
  
  // Convert ADC (0-1023) to voltage 0.00 - 5.00V
  float voltage = (cv_value * 5.0f) / 1024.0f;
  display.print(voltage, 2);  // 2 decimal places
  display.print("V -> ");
  display.print((cv_value * 12) / 1024);  // Step number
  ...
  // Output status (works for both MIDI and CV pulses)
  for (int i = 0; i < 12; i++) {
    int x = i * 10;
...
void loop() {
  ...
  handleMIDI();  // Extra polling for low latency
}