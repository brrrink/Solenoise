//---------------------- Solenoise Firmware Testing v10 -------------------------//
//----------------------------- John Harding -----------------------------------//
//---------------------------- Steal This Code ---------------------------------//
...
// - Testing:
// - Adding Encoder input for actuator on time plus gate delay (delays gate read to allow CV to settle)
...
int editMode = 0;            // 0 = normal, 1 = settle delay, 2 = pulse duration
...
void updateDisplay() {
  ...
  // Top line - base labels (always visible)
  display.setCursor(0, 0);
  display.setTextColor(WHITE);
  display.print("On:");
  display.setCursor(28, 0);           // Position after "On:"
  display.print(pulseDuration);
  display.print("ms");

  display.setCursor(68, 0);
  display.print("Del:");
  display.setCursor(96, 0);           // Position after "Del:"
  display.print(settleDelayUS);
  display.print("us");

  // Highlight pulse duration value
  if (editMode == 1) {
    display.fillRect(28, 0, 38, 8, WHITE);   // Box around "30ms"
  ...
  // Highlight settle delay value
  if (editMode == 2) {
    display.fillRect(96, 0, 32, 8, WHITE);   // Box around "200us"
    display.setTextColor(BLACK);
