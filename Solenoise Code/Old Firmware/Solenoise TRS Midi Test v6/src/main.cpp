...
const bool INVERT_ENCODER = false;  // Set to true if clockwise should increase count
...
int debouncedButtonState = HIGH;  // Track debounced state for edge detection
...
void readInputs() {
  ...
  int count = ((detent_position % 24) + 24) % 24; //ensuring count always falls in the range [0, 23]
  ...
  if ((millis() - lastDebounceTime) > SWITCH_DEBOUNCE_MS) {
    if (reading != debouncedButtonState) {
      ...
      if (debouncedButtonState == LOW) {  // Leading edge press
